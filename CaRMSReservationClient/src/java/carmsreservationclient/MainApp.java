...
public class MainApp {
    ...
    public MainApp(CarSessionBeanRemote carSessionBeanRemote, CustomerSessionBeanRemote customerSessionBeanRemote, ReservationSessionBeanRemote reservationSessionBeanRemote) {
    ...
    public void registerCustomer() {       
        ...
        catch(OwnCustomerExistException | UnknownPersistenceException ex) {
            System.out.println("An error has occurred while registering the new customer: " + ex.getMessage() + "!\n");
        }
    }
    
    public void customerLogin() throws InvalidLoginCredentialException, CustomerNotFoundException {
        Scanner scanner = new Scanner(System.in);
    ...
    public void searchCar() {
        try {
            ...
            List<Car> availableCars = carSessionBeanRemote.searchAvailableCars(pickupDate, pickupOutlet, returnDate, returnOutlet);
            System.out.printf("%15s%15s%15s%15s", "License Plate", "Colour", "Car Model", "Car Category", "Rental Rate");
            
            for(Car car : availableCars) {
                BigDecimal rentalRate = reservationSessionBeanRemote.calculateRentalRate(car, pickupDate, returnDate);
                System.out.printf("%15s%15s%15s", car.getLicensePlate(), car.getColour(), car.getCarModel().getModelName(), car.getCarModel().getCarCategory().getCategoryName(), rentalRate);
            }
            ...
            if(response == 1) {
                if(ownCustomer != null) {
                    ...
                    reserveCar(licensePlate, pickupDate, returnDate, pickupOutlet, returnOutlet);
                }
                else {
    ...
    public void reserveCar(String licensePlate, Date pickupDate, Date returnDate, String pickupOutlet, String returnOutlet) {
        try {
            Scanner scanner = new Scanner(System.in);
            ...
            Car reserveCar = carSessionBeanRemote.retrieveCarByLicensePlate(licensePlate);
            BigDecimal rentalRate = reservationSessionBeanRemote.calculateRentalRate(reserveCar, pickupDate, returnDate);
            ...
            if(input.equals("Y")) {
                ...
                newReservation.setIsPaid(Boolean.TRUE);
            }
            else {
                newReservation.setPaymentDate(pickupDate);
                newReservation.setIsPaid(Boolean.FALSE);
            }
            
            newReservation = reservationSessionBeanRemote.createNewReservation(newReservation, ownCustomer, licensePlate, pickupOutlet, returnOutlet);
            System.out.println("Car reserved successfully!: " + newReservation.getReservationId() + "\n");
        } catch (CarNotFoundException | OutletNotFoundException | UnknownPersistenceException ex) {
    ...
    public void viewReservationDetails() {
        try {
            ...
            System.out.printf("%5s%25s%25s%15s", reservation.getReservationId(), outputDateFormat.format(reservation.getReservationStartDate()),
                outputDateFormat.format(reservation.getReservationEndDate()), reservation.getTotalAmount(), reservation.getIsPaid());
